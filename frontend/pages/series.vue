...
useHead({ title: 'Series — Reelhouse' })
...
const query = computed(() => ({
  ...
  first_air_date_year: filters.value.year || undefined,
...
const { data, pending } = await useTmdb<{
  ...
  page: number
  total_pages: number
}>('discover/tv', () => query.value, { lazy: true })

const yearOf = (s: Series) => (s.first_air_date ? Number(s.first_air_date.slice(0, 4)) : null)
</script>

...
// TMDB season data — the FALLBACK source when MovieBox structure is
// unavailable. When structure loads, it (not this) drives the pickers.
const { data: seasonData, pending: seasonPending } = await useTmdb<{ episodes: Episode[] }>(
  () => `tv/${id.value}/season/${activeSeason.value ?? 1}`,
  {},
  { lazy: true },
)
const tmdbEpisodes = computed<Episode[]>(() => seasonData.value?.episodes ?? [])

// MovieBox's authoritative season/episode structure (numbering aligned with
// what we actually download from). This is the source of truth for the
// pickers; TMDB only supplies names (already overlaid server-side).
...
const useStructure = computed(() => !!structure.value?.seasons?.length)

type PickerSeason = { value: number; label: string }
const seasonOptions = computed<PickerSeason[]>(() => {
  if (useStructure.value) {
    return structure.value!.seasons.map((s) => ({
      value: s.se,
      label: `Season ${s.se} (${s.episodes.length} ep)`,
    }))
  }
  return seasons.value.map((s) => ({
    value: s.season_number,
    label: `${s.name} (${s.episode_count} ep)`,
  }))
})
...
const episodes = computed<UiEpisode[]>(() => {
  if (useStructure.value) {
    const s = structure.value!.seasons.find((x) => x.se === activeSeason.value)
    return (s?.episodes ?? []).map((e) => ({
      key: `mb-${activeSeason.value}-${e.ep}`,
      episode_number: e.ep,
      name: e.name,
      overview: e.overview ?? '',
  ...
  return tmdbEpisodes.value.map((e) => ({
    key: `tmdb-${e.id}`,
...
// True only while we have no episodes to show yet (initial load).
const episodesPending = computed(
  () => !episodes.value.length && (useStructure.value ? false : seasonPending.value),
)

// Default/repair the active season against whatever source is driving the
// picker — and reset to the first season if a URL-restored value isn't valid
// in MovieBox's numbering.
watch(
  seasonOptions,
  (opts) => {
    if (!opts.length) return
    if (activeSeason.value === null || !opts.some((o) => o.value === activeSeason.value)) {
      activeSeason.value = opts[0].value
    }
  },
  { immediate: true },
)
  ...
  <div>
    ...
    <template v-else>
          ...
          <div class="space-y-4">
            ...
            <div class="flex flex-wrap items-center gap-3 pt-2">
              ...
              <select
                ...
                class="rounded-md bg-ink-900 border border-white/10 px-3 py-2 text-sm h-[42px]"
              >
                <option v-for="e in episodes" :key="e.key" :value="e.episode_number">
      ...
      <section class="max-w-7xl mx-auto px-6 my-8">
        ...
        <div v-if="episodesPending" class="text-slate-400 py-8 text-center">Loading episodes…</div>
        <div v-else-if="!episodes.length" class="text-slate-400 py-8 text-center">No episodes.</div>
        <ul v-else class="divide-y divide-white/5 rounded-lg ring-1 ring-white/5 overflow-hidden">
          <li v-for="e in episodes" :key="e.key">
            <NuxtLink
